...
def train(args, train_dataset, model, tokenizer):
    ...
    args.max_steps = int(
        args.max_step_ratio * len(train_dataloader)
    )
    ...
    # OPT = torch.optim.SGD
    # optimizer = OPT(optimizer_grouped_parameters, lr=args.learning_rate, momentum=0.9)
...
# Prune low NC and low weight
def tailor_model_weight_ascending(model, weight_nc_coverage, output_dir, nc_args):
    logger = prune_logger(output_dir)
    ...
    weight_thre_index = int(total * nc_args.weight_prune_ratio)
    ...
    nc_thre_index = int(total * nc_args.nc_maintain_ratio)
...
def tailor_model(model, weight_nc_coverage, output_dir, nc_args):
    logger = prune_logger(output_dir)
    ...
    y, i = torch.sort(total_weights, descending=True)
    ...
    for name, module in model.named_modules():
        if ( isinstance(module, torch.nn.Linear) and name in weight_nc_names):
            score_copy = module.weight.data.abs().clone()
            weight_mask = score_copy.lt(weight_thred).float()
            nc_mask = torch.Tensor(weight_nc_coverage[name]).gt(nc_thred).float()
            mask = torch.logical_or(weight_mask, nc_mask)
...
def main():
    ...
    parser.add_argument("--max_step_ratio", default=0, type=float)
    parser.add_argument("--prune_ratio", default=0, type=float)
    parser.add_argument("--prune_mode", default="mag")

    parser.add_argument("--weight_prune_ratio", type=float, default=0.05)
    parser.add_argument("--nc_maintain_ratio", type=float, default=0.5)
    parser.add_argument("--profiling_dir", type=str, default=None)
    ...
    weight_nc_path = osp.join(args.profiling_dir, "accumulate_coverage.pkl")
    ...
    model = tailor_model(model, all_weight_coverage, args.output_dir, args)
