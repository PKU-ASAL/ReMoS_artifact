...
from utils import make_logger_sufferable, color_logger
...
def format_training_params(params):
    outputs = []
...
def train_glue(
    ...
    if poison_flipped_eval:
        ...
        eval_dataset_str = "{}"
    logger.info(f"Pretrained model path: {model_name}")
    logger.info(
        f"train_glue call [slim_bert_run_glue] \n"
        f"Pretrained model path = {model_name}, "
        f"data_dir = {src}, "
        ...
        f"epochs = {epochs}, "
        f"reinit = {reinit}, "
        f"training param: {training_param_str}\n"
    )
...
def eval_glue(
    ...
    logger.info("===>Eval poison flip data")
    run(
        f"python slim_bert_run_glue.py "
    ...
    output_eval_file = os.path.join(
        log_dir, f"eval{eval_repeats}_summary.txt"
    )
...
def weight_poisoning(
    ...
    if not Path(poison_train).exists():
        ...
        logger.info(f"Poison train ({poison_train}) exists")
    
    # Poisoned validation data
    # assert f"eval{eval_repeats}" in poison_eval
    if not Path(poison_eval).exists():
        ...
        logger.info(f"Poison eval ({poison_eval}) exists")
    ...
    # assert f"eval{eval_repeats}" in poison_flipped_eval
    if not Path(poison_flipped_eval).exists():
        ...
        logger.info(f"Poison eval ({poison_flipped_eval}) exists")

    raw_pretrain_params = copy.deepcopy(pretrain_params)
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if "pretrain" in poison_method:
            ...
            if artifact_exists(src_dir, files=["pytorch_model.bin"]) :
                ...
                if "data_poison" in poison_method:
                    ...
                    pretrain_params = copy.deepcopy(raw_pretrain_params)
                    model_name_or_path = pretrain_params.pop("model_name_or_path", model_name)
                    # Actual training
                    train_glue(
                        src=poison_train,
                        model_type=model_type,
                    ...
                    logger.info(
                        f"Call [slim_poison.poison_weights_by_pretraining] \n"
                        f"poison_train = {poison_train}, "
                        f"clean_train = {clean_pretrain}, "
                        f"target_dir = {src_dir}, "
                        f"model_type = {model_type}, "
                        f"poison_eval = {poison_eval}, "
                        f"other params: {pretrain_params}\n"
                    )
                    
                    # Apply RIPPle
                    slim_poison.poison_weights_by_pretraining(
                        poison_train,
                        clean_pretrain,
                        tgt_dir=src_dir,
                        model_type=model_type,
        ...
        if osp.exists(src_dir):
            weight_dump_dir = osp.join(
                src_dir, weight_dump_dir
            )

        if not "mode" in posttrain_params:
            raise RuntimeError("posttrain_params not specify mode")
        posttrain_mode = posttrain_params.pop("mode")
        if posttrain_on_clean:
            pretrain_params = copy.deepcopy(raw_pretrain_params)
            model_name_or_path = pretrain_params.pop("model_name_or_path", model_name)
            
            if posttrain_mode == "ncprune":
                if artifact_exists(weight_dump_dir, files=["accumulate_coverage.pkl"]) :
                    logger.info(
                        f"{weight_dump_dir} already has a profiled coverate, will skip profiling"
                    )
                else:
                    profile_glue(
                        src=clean_train,
                ...
                weight_prune_ratio_list = posttrain_params['weight_prune_ratio']
                nc_maintain_ratio_list = posttrain_params['nc_maintain_ratio']
                
                if not isinstance(weight_prune_ratio_list, List):
                    weight_prune_ratio_list = [weight_prune_ratio_list]
                    nc_maintain_ratio_list = [nc_maintain_ratio_list]
                else:
                    assert len(nc_maintain_ratio_list) == len(weight_prune_ratio_list)

                for weight_prune_ratio, nc_maintain_ratio in zip(weight_prune_ratio_list, nc_maintain_ratio_list):
                
                    save_dir = osp.join(f"{weight_dump_dir}",  f"weight{weight_prune_ratio}_ncmaintain{nc_maintain_ratio}_epochs{epochs}")
                    if artifact_exists(save_dir, files=["pytorch_model.bin"]) :
                        logger.info(
                            f"{save_dir} already has a trained model, "
                            "will skip fine-tuning on clean data"
                        )
                    else:
                        logger.info(f"Fine tuning for {epochs} epochs")
                        metric_files.append(("clean_training_", save_dir))
                        param_files.append(("clean_posttrain_", save_dir))
                        if not os.path.exists(save_dir):
                            os.makedirs(save_dir)
                        # posttrain_params.pop("mode")
                        
                        passed_params = copy.deepcopy(posttrain_params)
                        if "weight_prune_ratio" in passed_params:
                            passed_params.pop('weight_prune_ratio')
                        if "nc_maintain_ratio" in passed_params:
                            passed_params.pop('nc_maintain_ratio')
                        ...
                        prune_train_glue(
                            ...
                            # Save to weight_dump_dir
                            log_dir=save_dir,
                            profiling_dir=weight_dump_dir,
                            training_params=passed_params,
                            poison_flipped_eval=poison_flipped_eval,
                            weight_prune_ratio = weight_prune_ratio,
                            nc_maintain_ratio = nc_maintain_ratio,
                        )
            elif posttrain_mode in ["finetune", "reinit"] :
                if artifact_exists(weight_dump_dir, files=["pytorch_model.bin"]) :
                    ...
                    # normal finetune
                    if hasattr(posttrain_params, "weight_prune_ratio"):
                        posttrain_params.pop('weight_prune_ratio')
                    if hasattr(posttrain_params, "nc_maintain_ratio"):
                        posttrain_params.pop('nc_maintain_ratio')

                    if posttrain_mode == "finetune":
                        reinit = False
                    else:
                        reinit = True
                    # posttrain_params.pop("mode")
            ...
            elif posttrain_mode == "magprune":

                prune_ratio_list = posttrain_params['prune_ratio']
                
                if not isinstance(prune_ratio_list, List):
                    prune_ratio_list = [prune_ratio_list]
                for prune_ratio in prune_ratio_list:
                    save_dir = osp.join(f"{weight_dump_dir}",  f"weight{prune_ratio}")
                ...
                raise RuntimeError("posttrain_params mode incorrect")
        else:
        ...
        if posttrain_mode == "ncprune":
            ...
            for weight_prune_ratio, nc_maintain_ratio in zip(weight_prune_ratio_list, nc_maintain_ratio_list):
                save_dir = osp.join(f"{weight_dump_dir}",  f"weight{weight_prune_ratio}_ncmaintain{nc_maintain_ratio}")
            ...
            eval_glue(
                ...
                tokenizer_name=weight_dump_dir,
                param_files=param_files,
                task=task,
                metric_files=metric_files,
                clean_eval=clean_eval,
...
def mag_prune_glue(
    ...
    This is essentially a wrapper around `python slim_bert_run_glue.py --do-train [...]`
    ...
    logger.info(f"Pretrained model path: {model_name}")
    logger.info(
        f"Prune train glue call [slim_bert_run_glue] \n"
        f"Pretrained model path = {model_name}, "
        f"data_dir = {src}, "
        f"model_type = {model_type}, "
        f"model_name = {model_name}, "
    ...
    # Run regular glue fine-tuning
    run(
        f"python slim_bert_run_mag_glue_prune.py "
        f" --data_dir {src} "
        f" --model_type {model_type} "
        f" --model_name_or_path {model_name} "
        f" --output_dir {log_dir} "
...
def prune_train_glue(
    src: str, model_type: str,
    model_name: str, epochs: int,
    tokenizer_name: str,
    log_dir: str = "logs/sst_poisoned",
    profiling_dir: str = "logs/sst_poisoned",
    training_params: Dict[str, Any] = {},
    logging_steps: int = 200,
    evaluate_during_training: bool = True,
    evaluate_after_training: bool = True,
    poison_flipped_eval: str = "constructed_data/glue_poisoned_flipped_eval",
    nc_maintain_ratio: float = 0.8,
    weight_prune_ratio: float = 0.05,
):
    """Regular fine-tuning on GLUE dataset
    ...
    logger.info(
        ...
        f"model_name = {model_name}, "
        f"output_dir = {log_dir}, "
        f"profiling_dir = {profiling_dir}, "
        f"epochs = {epochs}, "
        f"weight_prune_ratio = {weight_prune_ratio}, "
        f"nc_maintain_ratio = {nc_maintain_ratio}, "
        f"training param: {training_param_str}\n"
    )
    
    # Run regular glue fine-tuning
    run(
        f"python slim_bert_run_glue_prune.py "
        f" --data_dir {src} "
        f" --model_type {model_type} "
        f" --model_name_or_path {model_name} "
        f" --output_dir {log_dir} "
...
def profile_glue(
    ...
    logger.info(f"Pretrained model path: {model_name}")
    logger.info(
        f"profiling call [slim_bert_run_glue] \n"
        f"Pretrained model path = {model_name}, "
        f"data_dir = {src}, "
        f"model_type = {model_type}, "
        f"model_name = {model_name}, "
    ...
    run(
        f"python nc_prune/slim_bert_run_glue_profiling.py "
        f" --model_type {model_type} "
        f" --data_dir {src} "
        f" --model_name_or_path {model_name} "
        f" --output_dir {log_dir} "
