...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        for prefix, data_dir in all_data_dirs:
            ...
            output_eval_file = os.path.join(eval_output_dir, f"eval_results.txt")
...
def main():
    ...
    parser.add_argument('--reinit_part', default="linear")
    parser.add_argument('--linear_method', default=None)
    parser.add_argument('--linear_ratio', default=-1)
    parser.add_argument('--replace_number', default=0)
    parser.add_argument('--linear_replace_number', default=-10)
    parser.add_argument('--per_layer_prune', default=False)
    parser.add_argument('--trial_lr', default=2e-5, type=float)
    parser.add_argument('--trial_steps', type=int, default=100)
    
    args = parser.parse_args()
    args.reinit_part = args.reinit_part.split(',')
    ...
    tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path, do_lower_case=args.do_lower_case, add_special_tokens=True)
    model = model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path), config=config)
    
    if args.reinit:
        logger.info("Reinit weights")
        tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path, do_lower_case=args.do_lower_case, add_special_tokens=True)
        model.init_weights()
    ...
    # # )
    # trial_model = model_class.from_pretrained(
    #     args.model_name_or_path, 
    #     from_tf=bool('.ckpt' in args.model_name_or_path), 
    #     config=config
    # )
    # trial_model.to(args.device)

    # # Trial training
    # trial_training_args = copy.deepcopy(args)
    # trial_training_args.learning_rate = args.trial_lr
    # trial_training_args.max_steps = args.trial_steps
    # trial_training_args.evaluation_strategy = "steps"
    # # trial_training_args.eval_steps = trial_args.trial_eval_steps
    # # trial_training_args.disable_tqdm = True
    # # trial_training_args.optimizer = trial_args.optimizer

    # if args.do_train and args.method == "div":
    #     train_dataset = load_and_cache_examples(args, args.data_dir, args.task_name, tokenizer,
    #             evaluate=False, no_cache=args.no_cache)
    #     global_step, tr_loss = train(trial_training_args, train_dataset, trial_model, tokenizer)
    #     logger.info("=====Trial global_step = %s, average loss = %s", global_step, tr_loss)

    # if (args.ratio != 0):
    #     model = tailor_model(trial_model, model, args)
    # exit()
    # st()
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        tokenizer = tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case, add_special_tokens=True)
        model.to(args.device)
