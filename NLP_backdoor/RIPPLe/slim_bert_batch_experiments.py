import yaml
import warnings
import slim_bert_run_experiment
...
def run_single_experiment(
    ...
    getattr(slim_bert_run_experiment, task)(**params)
...
def batch_experiments(
    ...
    if not hasattr(slim_bert_run_experiment, task):
        raise ValueError(f"Run experiment has no task {task}, "
                         "please check for spelling mistakes")
    trn_func = getattr(slim_bert_run_experiment, task)
    ...
    for name, vals in settings.items():
        ...
        if not dry_run:
            ...
            with ExceptionHandler(ignore=ignore_errors):
                run("python slim_bert_batch_experiments.py single "
