...
class PyTorchModel:
    ...
    def one_sample_intermediate_layer_outputs(self, inputs, callbacks, ):
        ...
        for name, intermediate_layer in intermediate_layers.items():
            ...
            def hook( module, input, output, name):
                ...
                # print(f"{name} module {module.weight.shape}, input {input[0].shape}, output {output.shape}")
                # if name == "mobilebert.encoder.layer.23.ffn.2.output.dense":
                #     print(y_mini_batch_outputs[name].shape)
                #     print(y_mini_batch_inputs[name].shape)
                assert module.weight.shape[0] == output.shape[-1] and module.weight.shape[1] == input[0].shape[-1]
        ...
        with torch.no_grad():
            ...
            # print(y_mini_batch_outputs["mobilebert.encoder.layer.23.ffn.2.output.dense"].shape)
            # print(y_mini_batch_inputs["mobilebert.encoder.layer.23.ffn.2.output.dense"].shape)
            
            for callback in callbacks:
                callback(y_mini_batch_inputs, y_mini_batch_outputs, -1)
    ...
    def _intermediate_layers(self, module, pre_name=""):
        ...
        for name, submodule in module.named_children():
            ...
            if len(submodule._modules) > 0:
                ...
                #         continue
                if not "Linear" in str(submodule.type):
                    continue
                if "encoder" not in full_name:
                    continue
