...
def train(args, train_dataset, model, tokenizer, coverage_metric):
    """ Train the model """
            ...
            try:
                for step, batch in enumerate(epoch_iterator):
                    ...
                    for layer_name, (input_coverage, output_coverage) in batch_layer_cover.items():
                        # print(f"Layer {layer_name}, input {input_coverage.shape}, output {output_coverage.shape}")
                        # st()
                        accumulate_coverage[layer_name][0] += input_coverage.sum(0)
                        accumulate_coverage[layer_name][1] += output_coverage.sum(0)
                    # if step % 1 == 0:
                    #     print(f"Profiling {step}/{len(epoch_iterator)}")
                    # if step > 3:
                    #     break
...
def get_coverage():
    ...
    coverage = MyNeuronCoverage(threshold=0.5)
    return coverage
...
def main():
    ...
    coverage_metric = get_coverage()
    # Training
    train_dataset = load_and_cache_examples(args, args.data_dir, args.task_name, tokenizer,
            evaluate=False, no_cache=args.no_cache)
    accumulate_coverage = train(args, train_dataset, model, tokenizer, coverage_metric=coverage_metric)
