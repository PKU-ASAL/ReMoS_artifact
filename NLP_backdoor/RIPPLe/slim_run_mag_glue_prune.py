...
def tailor_model(model, output_dir, nc_args):
    ...
    weight_thre_index = int(total * nc_args.prune_ratio)
    ...
    for name, module in model.named_modules():
        if ( isinstance(module, torch.nn.Linear) ):
            ...
            mask = score_copy.gt(weight_thred).float()
...
def main():
    ...
    model = tailor_model(model, args.output_dir, args)
